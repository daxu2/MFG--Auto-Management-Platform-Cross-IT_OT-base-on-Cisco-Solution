...
mytimeinterval = 30
runcount =
mytoken = "ics-e8fbcfb8cd54c6d70fba097b208f5115d5957c99-dbdd29d584c19dd2a37c8e71ee72a8310f74a586"


def get_componentbysite(ip,ver,uname,pword):
    ticket = get_X_auth_token(ip,ver,uname,pword)
    #headers = {"X-Auth-Token": myticket}
    headers = {'content-type':'application/json','X-Auth-Token':ticket}
    url = "https://"+ip+"/api/3.0/components"
...
def get_activitylist(ip,ver,uname,pword):
    print('get_devicelist')
    ticket = get_X_auth_token(ip,ver,uname,pword)
    #headers = {"X-Auth-Token": myticket}
    headers = {'content-type':'application/json','X-Auth-Token':myticket}
    url = "https://"+ip+"/api/3.0/activities"
    #print ("\nExecuting get_activitylist:\n")
    #print(url)
...
def componentetailbymacaddr(ip,ver,uname,pword,macaddr):
...
def getcomponentinfo(ip,ver,uname,pword):
    dnac_device = {}
...
def getpresets(ip,ver,uname,pword):
...
def main(argv):
  ...
  if (runcount == 0) :
     print('Run forever\n')
     while True:
        getpresets(m_APICEM_IP,VERSION,m_USERNAME,m_PASSWORD)
        time.sleep(1)
        getcomponentinfo(m_APICEM_IP,VERSION,m_USERNAME,m_PASSWORD)
        time.sleep(mytimeinterval*60)
  else :
     while (runcount > 0) :
       get_componentbysite(m_APICEM_IP,VERSION,m_USERNAME,m_PASSWORD)
       get_activitylist(m_APICEM_IP,VERSION,m_USERNAME,m_PASSWORD)
       runcount = runcount -1
       if (runcount == 0):
